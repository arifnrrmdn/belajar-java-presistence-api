...
public class CriteriaTest {
    ...
    void criteriaWhereClause() {
        ...
        criteria.where(
                builder.equal(b.get("name"), "Apple"),
    ...
    void criteriaNamedParameter() {
        ...
        ParameterExpression<String> brandPramater = builder.parameter(String.class, "brand");

        // select p from Product p join p.brand b

        criteria.select(p);
        criteria.where(
                builder.equal(b.get("name"), brandPramater)
        );
        // select p from Product p join p.brand b where b.name = 'Samsung'

        TypedQuery<Product> query = entityManager.createQuery(criteria);
        query.setParameter(brandPramater, "Xiaomi");
        List<Product> resultList = query.getResultList();
        for (Product product : resultList){
            System.out.println(product.getId() + " : " + product.getName());
        }
    ...
    void criteriaNonQuery() {
        ...
        criteria.set(b.get("description"), "Apple Solution");
